/* GPL-3.0-or-later. Combined host derived from the reviewed Task3 Summary.
 * The historical Summary source/build are preserved. The presentation and raw
 * lease helpers are shared unchanged. One 64-byte owner-tagged state is used
 * by the mutually exclusive top-level Summary and Oak lifetimes. */
/* Keep the reviewed raw lease implementation exact. Its heap19-only resolver
 * is superseded here by the same checked algorithm with an explicit heap ID. */
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunused-function"
#include "lease.c"
#pragma clang diagnostic pop
#define GUIDE_COPY_DECL __attribute__((noinline)) static
#define GUIDE_TEXT_ADDRESS 0x01FF9B10
#include "guide_present.c"
#define DISPLAY (*(volatile u32 *)0x04001000)
#define VRAM ((void *)(0x06210000u+(oak_owner()?0x8000u:0)))
#define GPU_MAP ((void *)(0x0620F800u-(oak_owner()?0x8000u:0)))
...
#define OAK_MAGIC 0x4F414B31u
#define BRIGHTNESS (*(volatile u16 *)0x0400106C)
#define BLEND ((volatile u16 *)0x04001050)
/* Oak-only bytes follow the modal bitmap, never the smaller Summary badge. */
typedef struct { u8 glyph[64];u16 brightness,blend[3]; } OakScratch;
static int oak_owner(void) { return P->magic==OAK_MAGIC; }
static u8 *host_bg(u8 *s) { return *(u8 **)(s+(oak_owner()?0x18:0)); }
...
_Static_assert(sizeof(Context)+2*MODAL_TILES*32+2048+sizeof(OakScratch)+4<=LEASE_BYTES,"Modal cap");
_Static_assert(sizeof(Context)+2*20*32+40<=BADGE_BYTES,"Badge cap");

static u8 *font_work(void) { return *(u8 **)0x0211188C; }

static int summary_resources(u8 *s)
{
...
static int resources(u8 *s) { return oak_owner()?oak_resources(s):summary_resources(s); }
static void schedule(u8 *s) { *(u16 *)(host_bg(s)+6)|=16; }
static int settled(u8 *s) { return !(*(u16 *)(host_bg(s)+6)&16); }
...
static OakScratch *oak_scratch(Context *c) { return (OakScratch *)(bitmap(c)+MODAL_TILES*32); }
...
static void font_save(Context *c)
{
    ...
    if(oak_owner()) copy16(oak_scratch(c)->glyph,*(u8 **)(font_work()+0x9C)+0x14,64);
}
static void font_restore(Context *c)
{
    ...
    if(oak_owner()) copy16(*(u8 **)(font_work()+0x9C)+0x14,oak_scratch(c)->glyph,64);
}

static void draw_window(Context *c,u32 x,u32 y,u32 w,u32 h,u32 first,u32 id,u32 disabled)
{
    GuideWindow *v=&c->window;
    v->bg=host_bg(P->summary); v->id=4;v->x=x;v->y=y;v->w=w;v->h=h;v->pal=13+oak_owner();v->tile=first;
    v->pixels=bitmap(c)+(first-c->first)*32;
    u32 background=14+oak_owner();
    fill32(v->pixels,0x11111111u*background,w*h*32);
    COLORS(disabled?(oak_owner()?2:15):1,background,disabled?background:2);
    guide_line(v,id,(id>=15 && id!=20)?(w*8-TEXT[24+id])/2:0,0);
    for(u32 row=0;row<h;row++) for(u32 col=0;col<w;col++)
        c->map[(y+row)*32+x+col]=((u32)v->pal<<12)+first+row*w+col;
}

static Context *acquire(u8 *s,u32 badge)
{
    Lease lease={host_heap(oak_owner()?80:19),0};
    ...
    c->map=*(u16 **)(host_bg(s)+0xB8);c->bytes=badge?BADGE_BYTES:LEASE_BYTES;
    ...
    if(oak_owner()) {
        OakScratch *o=oak_scratch(c);o->brightness=BRIGHTNESS;
        copy16(o->blend,(void *)BLEND,6);
    }
    return c;
}
...
static void restore(Context *c)
{
    ...
    schedule(P->summary);
    if(oak_owner()) {
        OakScratch *o=oak_scratch(c);BRIGHTNESS=o->brightness;
        copy16((void *)BLEND,o->blend,6);
    }
}
...
/* GUIDA-EVIV-02, opzione (b) del rapporto GUIDA-EVIV-01: l'etichetta automatica
 * "START Guida" non si disegna piu'. Restano SOLO le tre assegnazioni di stato
 * che servono a P->badge (letto da guide_main come cancello interno di START/
 * tocco) e alla contabilita' di P->summary/P->visibility/P->pending: niente
 * acquire()/font_save/draw_window/border/font_restore/copy16/schedule, quindi
 * P->context resta 0 e badge_remove()/restore()/guide_cleanup() (tutti
 * condizionati su P->context) non toccano mai una finestra mai disegnata. Le
 * guardie di eleggibilita' (righe sopra) restano invariate. */
...
static void modal_draw(Context *c)
{
    u32 panel=P->panel;
    font_save(c);
    fill32(bitmap(c),oak_owner()?0xFFFFFFFF:0xEEEEEEEE,MODAL_TILES*32);
    for(u32 i=0;i<1024;i++) c->map[i]=((13u+oak_owner())<<12)+1;
    ...
    draw_window(c,2,20,9,2,240,16,panel==0);border(&c->window,panel==0?2:(oak_owner()?1:15));
    draw_window(c,12,20,8,2,258,panel==2?18:17,0);border(&c->window,oak_owner()?1:15);
    draw_window(c,22,20,8,2,274,19,0);border(&c->window,oak_owner()?1:15);
    font_restore(c);
...
{
    u16 *cpu=*(u16 **)(host_bg(s)+0xB8);volatile u16 *gpu=GPU_MAP;
...
static int modal_run(u8 *s)
{
    ...
    if(P->phase==3) {
        if(settled(s)) { if(oak_owner()) { BRIGHTNESS=0;BLEND[0]=0; } visibility(256);P->phase=4; }
        return 1;
    }
    if(P->phase==4) {
