...
def load_text(data, address, entries=()):
    ...
    for section in sections:
        ...
        for offset, info in struct.iter_unpack('<II', content(section)):
            ...
            require(offset + 4 <= len(result), 'Rilocazione fuori da .text')
            tipo = info & 255
            # R_ARM_ABS32 (2) su un simbolo INTERNO a .text: e' la parola del pool
            # letterale con cui il compilatore raggiunge una funzione lontana.
            # SGP-1.2-OPZIONI-01 la ammette — e SOLO questa forma — perche' il
            # payload dell'interfaccia e' piu' grande di quello di D1 e clang la
            # emette da solo. Resta vietato tutto il resto (simboli esterni, altri
            # tipi, sezioni allocate oltre .text): i controlli sopra non cambiano.
            # Il valore del simbolo porta gia' il bit Thumb (clang lo imposta per
            # le funzioni Thumb), quindi non va aggiunto a mano.
            if tipo == 2:
                addend = struct.unpack_from('<I', result, offset)[0]
                struct.pack_into('<I', result, offset,
                                 (address + symbol[1] + addend) & 0xFFFFFFFF)
                continue
            require(tipo == 10,
                    'Ammesse solo BL Thumb (R_ARM_THM_CALL) e ABS32 interne a .text')
