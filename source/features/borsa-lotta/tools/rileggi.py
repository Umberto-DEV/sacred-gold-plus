#!/usr/bin/env python3
"""Rilettore indipendente del gancio ``sgp.borsa_lotta``.
...
BASE, N_BLOCCO = 0x023DBD00, 0x100
MAX_CODICE, OFF_CANARINO, N_CANARINO = 0xF0, 0xF0, 16
OVERLAY, SITO = 8, 0x02223C30
PRE = bytes.fromhex("54f6aaf8")
NATIVO = 0x02077D88
NOME_GANCIO = "sgp_borsa_attr_hook"
NOME_CACHE = "sgp_borsa_attr_cache"
CANARINO_FISSO = b"".join(struct.pack("<I", 0xCA5A1900 | i) for i in range(4))
...
def _arm9(raw):
    td = tempfile.TemporaryDirectory(prefix="borsa-lotta-ro-")
...
def _rileggi(prima, dopo, build_dir):
    ...
    forma = (man.get("base") == "0x023dbd00" and
             ...
             man.get("gancio") == {"overlay": OVERLAY,
                                    "sito": "0x02223c30",
                                    "pre": PRE.hex(),
                                    "bersaglio": NOME_GANCIO} and
             isinstance(man.get("simboli"), dict) and
             set(man["simboli"]) == {NOME_GANCIO, NOME_CACHE})
    ck("B0", forma, "forma e impronte del bundle")

    simboli = {nome: int(man["simboli"][nome], 16)
               for nome in (NOME_GANCIO, NOME_CACHE)}
    ...
    try:
        ...
        off = SITO - oa["ram"]
        pre_ok = ia[off:off + 4] == PRE and bersaglio_bl(SITO, PRE) == NATIVO
        post = ib[off:off + 4]
        post_ok = bersaglio_bl(SITO, post) == (simboli[NOME_GANCIO] & ~1)
        ck("L3", pre_ok and post_ok,
           "BL nativa e BL derivata decodificate ai bersagli attesi")

        diffov = ([i for i, (x, y) in enumerate(zip(ia, ib)) if x != y]
                  if len(ia) == len(ib) else [])
        finestra = set(range(off, off + 4))
        inattesi_ov = [i for i in diffov if i not in finestra]
        ck("L4", len(ia) == len(ib) and 1 <= len(diffov) <= 4 and not inattesi_ov,
