#!/usr/bin/env python3
"""Compila il gancio Borsa: codice Thumb e canarino, nessun asset di gioco."""
import argparse
import hashlib
import json
...
def compila(out):
    ...
    source = HERE.parent / 'sorgenti/borsa_cache.c'
    flags = ['--target=armv5te-none-eabi', '-mcpu=arm946e-s', '-mthumb', '-Oz',
             '-ffreestanding', '-fno-builtin', '-fno-stack-protector',
             '-fno-unwind-tables', '-fno-asynchronous-unwind-tables',
             '-fno-jump-tables', '-Wall', '-Wextra', '-Werror']
    with tempfile.TemporaryDirectory(prefix='sgp-borsa-compile-') as temp:
        obj = Path(temp) / 'borsa_cache.o'
        subprocess.run(['clang', *flags, '-c', str(source), '-o', str(obj)], check=True)
        blob, symbols = load_text(obj.read_bytes(), 0x023DBD00,
                                  ('sgp_borsa_attr_cache', 'sgp_borsa_attr_hook'))
    if len(blob) > 240:
        raise ValueError('codice oltre il canarino a +0xF0')
    canary = b''.join((0xCA5A1900 | i).to_bytes(4, 'little') for i in range(4))
    (out / 'blob.bin').write_bytes(blob)
    (out / 'canarino.bin').write_bytes(canary)
    manifest = {
        'schema': 1, 'base': '0x023dbd00', 'blocco_byte': 256,
        'canarino_offset': 240,
        'blob': {'byte': len(blob), 'sha256': sha(blob)},
        'canarino': {'byte': len(canary), 'sha256': sha(canary)},
        'simboli': {k: hex(symbols[k]) for k in ('sgp_borsa_attr_cache', 'sgp_borsa_attr_hook')},
        'gancio': {'overlay': 8, 'sito': '0x02223c30', 'pre': '54f6aaf8',
