#!/usr/bin/env python3
"""Contratto e mutanti del rilettore indipendente ``sgp.borsa_lotta``."""
...
BUILD = REPO / "source/sgp12/build/borsa_lotta"
PRIMA = Path(os.environ.get("SGP_ROM_BORSA_PRIMA", ""))
DOPO = Path(os.environ.get("SGP_ROM_BORSA_DOPO", ""))


def carica():
    spec = importlib.util.spec_from_file_location(
        "borsa_lotta_rilettore", PACCHETTO / "tools/rileggi.py"
    )
...
class DecodificaBlTest(unittest.TestCase):
    def test_decodifica_la_preimmagine_nativa(self):
        self.assertEqual(reader.bersaglio_bl(0x02223C30, bytes.fromhex("54f6aaf8")),
                         0x02077D88)

    def test_rifiuta_un_opcode_che_non_e_bl_thumb(self):
        self.assertIsNone(reader.bersaglio_bl(0x02223C30, b"\0\0\0\0"))


@unittest.skipUnless(PRIMA.is_file() and DOPO.is_file() and
                     (BUILD / "manifesto.json").is_file(),
                     "servono SGP_ROM_BORSA_PRIMA/DOPO e la build borsa_lotta")
class MutantiClasseBTest(unittest.TestCase):
    ...
    def copia_build(self):
        td = tempfile.TemporaryDirectory(prefix="borsa-lotta-reader-")
    ...
    def guasta_arm9(self, raw, addr):
        td = tempfile.TemporaryDirectory(prefix="borsa-lotta-rom-")
        self.addCleanup(td.cleanup)
    ...
    def test_mutanti_blocco_e_bundle_sono_rossi(self):
        ...
        mutanti = {
            "codice": self.guasta_arm9(self.dopo, 0x023DBD00),
            "margine_zero": self.guasta_arm9(self.dopo, 0x023DBD00 + blob_n),
            "canarino": self.guasta_arm9(self.dopo, 0x023DBDF0),
        ...
        man["simboli"]["sgp_borsa_attr_hook"] = "0x023dbd01"
