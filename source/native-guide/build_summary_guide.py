#!/usr/bin/env python3
"""Build isolated native Summary-guide composition from exact Community1 Plus bases."""
import argparse
import json
...
STATE=0x01FF9FC0
END=0x01FFA000
SOURCES=['summary_guide.c','guide_present.c','lease.c','build_summary_guide.py','guide-abi.json','presentation-text.json','lease-abi.json','summary_pressure_test.h','guide-source-evidence.json']
...
def check_budget(payload,text):
    ...
    require(688+2*289*32+2048<=22528,'Modal heap budget overflow')
...
def extend(main,payload,symbols,text):
    ...
    for name in ['eviv_hook','guide_exit_hook']:
...
def build(source,charmap,out,pressure=False):
    ...
    obj=out/'guide.o'
    command=['clang','--target=armv5te-none-eabi','-mcpu=arm946e-s','-mthumb','-Os','-ffreestanding','-fno-builtin','-fno-stack-protector','-fno-unwind-tables','-fno-asynchronous-unwind-tables','-fno-jump-tables','-Wall','-Wextra','-Werror','-c',str(HERE/'summary_guide.c'),'-o',str(obj)]
    ...
    allowed=set(range(0x88424,0x8842c))|set(range(0x8856c,0x88574))|set(range(0xd2c68,0xd2c6c))|set(range(0xba0,0xba8))|set(range(0xbb4,0xbb8))
    ...
    target=out/f'Native Summary Guide TEST {lang.upper()}.nds';target.write_bytes(image)
    require(digest(source.read_bytes())==base_sha,'Base input changed')
    meta={'kind':'private-native-summary-guide','language':lang,'rom_file':target.name,'rom_sha256':digest(image),'base_sha256':base_sha,'r5_sha256':r5_sha,
          ...
          'text_sha256':digest(text),'text_bytes':len(text),'text_lines':lines,'abi':evidence,'memory':{'code_start':CODE,'text_start':TEXT,'state_start':STATE,'arena_low':END,'badge_bytes':2048,'modal_bytes':22528,'modal_tiles':289},'r5_and_other_rom_content_preserved':True,'pressure_test_instrumentation':pressure}
