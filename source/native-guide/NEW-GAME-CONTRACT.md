...
7 settembre 2026. **Contratto verificato sui binari e sul primo setup nativo EN/IT; nessuna integrazione New Game.** La presentazione Summary Task3 è ancora in sviluppo/review. Questo documento non ne anticipa l’accettazione. Metadati e piano (private lab record, not distributed); [presentazione condivisa](PRESENTATION.md).
...
Le prove storiche di banco (registro non distribuito) e gli script `boot-save.script`/`cold-reload.script` coprono SAVE/reload di una partita TEST già creata a Borgo Foglianova/New Bark Town, non la prima scrittura ottenuta da questa Nuova partita. Resta da osservare sulla futura candidata e sulla propria baseline: conferma nome→Oak Exit→posizione iniziale→primo SAVE→cold boot da quel save; confronto nome/genere/ID/progressione, uscita B anticipata/completa e nessuna ripetizione dopo naming. Non trasferire savestate fra ROM.

Strumenti delle misure: ndspy, hashlib e struct per leggere la ROM, clang e llvm-objdump per il disassemblaggio, l'observer del banco headless con gli script di localizzazione e di preparazione oltre allo script keyboard. Sei avvii in tutto: due di localizzazione EN (esplorativi), due di preparazione EN/IT e due keyboard EN/IT. Le analisi ndspy vogliono le dipendenze di `source/requirements.txt`. Nessun salvataggio originale è stato aperto e nessun byte di gioco è stato ridistribuito.
