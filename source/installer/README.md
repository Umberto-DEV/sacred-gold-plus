...
The release data has schema `1`, version `1.1`, `target` and exactly eight `sources`. The target contains `id`, `language` (`IT` or `US`), `camera` (always `Plus`: the camera is no longer a ROM variant, only an optional cheat), `sha256`, `bytes`, and `output_name` such as `Sacred Gold Plus 1.1 IT.nds` — the camera takes no part in the file name. The source IDs are `clean-us` for unmodified US HeartGold, `plus-1.01`/`plus-1.02`/`plus-1.03` for the supported English Plus versions, and `plus-1.04-en-plus`/`plus-1.04-en-classic`/`plus-1.04-it-plus`/`plus-1.04-it-classic` for the four 1.04 Community builds; each has `label`, `sha256`, `bytes`, and `patch: {sha256, bytes, data}` with base64 data. Labels are display text, never an identification mechanism. The reviewed packager pins the real source and target fingerprints; this generic client validates the embedded contract rather than asserting that arbitrary metadata is an official release.
