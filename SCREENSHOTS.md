# See Sacred Gold Plus

These are real captures from the 1.04 preview, at the Nintendo DS's native resolution with both screens visible. They use a test character and contain no personal save information. Preview 2 produces the same game as Preview 1.

## Exploring New Bark Town

![Exploring New Bark Town in the English 1.04 preview](images/01-new-bark-town.png)

The English version, using the wider New Angle camera. This is an existing town, not a new area added in 1.04.

## Choose your camera

| Normal Angle | New Angle |
| --- | --- |
| ![Normal Angle in New Bark Town](images/02-normal-angle.png) | ![New Angle at the same position in New Bark Town](images/03-new-angle.png) |
| The original HeartGold camera. | The wider view retained from Plus 1.03. |

Same scene, same position, same gameplay and fixes. Neither option is advertised as faster.

## Italian dialogue and naming

| Italian dialogue | Accented characters |
| --- | --- |
| ![Italian dialogue with Professor Oak and Marill](images/04-italian-dialogue.png) | ![The Italian naming keyboard with accented characters entered](images/05-italian-keyboard.png) |

The introduction is the existing one translated into Italian. The naming screen shows accented characters entered through the keyboard. Further language review and full-game testing are still welcome.

These captures show the selected screens, not a completed playthrough or a performance benchmark. The game artwork belongs to its original creators; see the [credits](CREDITS.md).

[Choose your download](README.md#download-104) · [Read the game guide](GAME-GUIDE.md) · [Report a problem](https://github.com/Umberto-DEV/sacred-gold-plus/issues/new/choose)
